...
/**
 * This Handler opens the CompareEngineView and sets the models from selection (source model and target model).
 * @author {Kamil Rosiak}
 *
 */
public class StartCompareEngineHandler {
	...
	public void execute(Shell shell, ServiceContainer services) {
		...
		if(selectedFiles.size() == 2) {
			boolean isConfiguration = checkForConfigurations(selectedFiles);
			if(isConfiguration) {
				...
				services.eventBroker.send(PPUEventTable.START_COMPARE_ENGINE, models);
			} else {
	...
	/**
	 * This method check if the selected files are from type configuration.
	 */
