...
import de.tu_bs.cs.isf.familymining.ppu_iec.core.util.ModelStatisticsUtil;
...
public class ExportStatistics {
	...
	public void execute(Shell shell, ServiceContainer services) {
		...
		if(selectedFiles.size() == 1) {
			...
			if(checkForConfigurations(selectedFiles)) {
				...
				ModelStatisticsUtil.printModelStatistics(models.get(0));
			} else {
	...
	public boolean canExecute(RCPSelectionService selectionService) {
		...
		return checkForConfigurations(selectedFiles) && selectedFiles.size() == 1;
	}
}
