...
public class ST_FunctionCallParameterType extends AbstractSTImplAttribute{
	/**
	 * 
	 */
	private static final long serialVersionUID = 5804049127623945449L;
	public static final String UNIT_NAME = "ST FunctionCallParamterType Attribute";
	public static final String UNIT_DESCRIPTION = "Compares the typs of the parameters";
	
	public ST_FunctionCallParameterType() {
		...
		if(source instanceof FunctionCallStatement && target instanceof FunctionCallStatement) {
			...
			while(sourceIterator.hasNext()) {
				...
				while(targetIterator.hasNext()) {
					...
					Expression sourceValue = currentSourceType.getAssignedValue();
					Expression targetValue = currentTargetType.getAssignedValue();
					if(sourceValue.getDataType().equals(targetValue.getDataType())) {
						paramSimilarity +=1f;
